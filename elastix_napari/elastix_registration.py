...
# Retrieves the points from the specified layer, and copies them into an `itk::VectorContainer` of `itk::Point` elements, and returns the `itk::VectorContainer`.
def _get_points_from_layer(points_layer: "napari.layers.Points"):
    point_set = point_set_from_points_layer(points_layer)
    tuple_of_points = point_set.GetPoints().CastToSTLConstContainer()
    new_point_set = type(point_set).New()
    vector_container = new_point_set.GetPoints()
    std_vector = vector_container.CastToSTLContainer()
    std_vector.reserve(len(tuple_of_points))

    # The coordinates of the point set returned by point_set_from_points_layer still need to be reversed, manually, when using itk_napari_conversion version 0.5.0.
    for point in tuple_of_points:
        std_vector.push_back(tuple(reversed(point)))
        ...
        if use_corresponding_points:
            ...
            if fixed_points is None:
                ...
                if fixed_points.data.size > 0:
                    kwargs["fixed_points"] = _get_points_from_layer(fixed_points)
                else:
                    return utils.error(
            ...
            if moving_points is None:
                ...
                if moving_points.data.size > 0:
                    kwargs["moving_points"] = _get_points_from_layer(moving_points)
                else:
                    return utils.error(
